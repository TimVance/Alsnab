...
if ($arResult["show"] == "order"): ?>
    <? if (!empty($arResult["items"])): ?>
        ...
        <div class="mng-order">
            ...
            <? if (!empty($arResult["handle"]) && $is_admin == 1): echo '<div>Редактировали:</div>'; ?>
                <? foreach ($arResult["handle"] as $handle): ?>
                    <div><? echo $handle["user"]["LOGIN"]."(id: ".$handle["user"]["ID"].") ".$handle["user"]["NAME"]." ".$handle["user"]["LAST_NAME"]; if (!empty($handle["bill"])) echo ' <a download href="'.$handle["bill"].'">Скачать счет</a>'; ?></div>
                <? endforeach; ?>
            <? endif; ?>
            <br>
            <form method="post" enctype="multipart/form-data">
                ...
                <? if (!$arResult["is_admin"]): ?>
                    <div class="upload_files">
                        <? if (!empty($arResult["handle"][$user_id]["bill"])): ?>
                            <div>Прикрепленный счет: <a download href="<?=$arResult["handle"][$user_id]["bill"]?>">скачать</a></div>
                            <h3>Прикрепить новый счет:</h3>
                        <? else: ?>
                            <h3>Прикрепить счет:</h3>
                        <? endif; ?>
